...
// DESIGN.md-compliant minimal theme for hero use.
// Pure near-black canvas background + very low-alpha white/ink particles only.
// No colored hues, no atmospheric "ocean/aurora" treatments.
const THEMES: Record<ColorTheme, ThemeConfig> = {
  aurora: {
    hueStart: 0,
    hueRange: 0,
    saturation: 0,
    lightness: 98, // near-white / ink
    bg: "10, 10, 10", // exact #0a0a0a canvas
    trailAlpha: 0.035,
  },
  ember: {
    hueStart: 0,
...
// DefaultContent is kept only for the component's own demo usage.
// It contains forbidden styles (font-bold + gradient text) per DESIGN.md.
// In the museum we always pass children from HeroSection (which uses compliant primitives).
// This block is intentionally left minimal / not used in production paths.
...
export default function FlowField({
  ...
  useEffect(() => {
    ...
    const resize = () => {
      // Size to parent container (for contained hero use) or fallback to viewport
  ...
  return (
    <div
      className={cn(
        "relative flex w-full items-center justify-center overflow-hidden min-h-[420px] md:min-h-[520px]",
        className
      )}
    ...
    >
      ...
      {/* DESIGN.md: no atmospheric gradients, vignettes, or fades.
          Particles are the only visual treatment (now forced to low-alpha ink on pure canvas). */}
